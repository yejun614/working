...
이메일 · 캘린더 · 문서 · 칸반 보드 · 시계를 하나의 가벼운 앱에서.
...
**Working**은 이메일, 캘린더, 문서, 칸반 보드, 시계를 한 곳에서 쓸 수 있는
...
- **시계**: 현재 시간과 세계 시간, 스톱워치·타이머·뽀모도로를 한 화면에서
사용합니다.
...
| 기능     | 설명                                          |
| ------ | ------------------------------------------- |
| 현재 시간  | 초 단위 로컬 시각과 날짜, 시간대 표시                      |
| 세계 시간  | IANA 시간대를 골라 추가, 현지 시각·날짜와 내 시간과의 시차 표시     |
| 스톱워치   | 1/100초 단위 측정, 랩 기록(구간·누적)                   |
| 타이머    | 시·분·초 입력과 빠른 프리셋, 진행 막대, 종료 시 알림음           |
| 뽀모도로   | 집중·짧은 휴식·긴 휴식 순환, 라운드 표시, 길이 설정 저장          |
| 시간 정확도 | 틱을 세지 않고 목표 시각과의 차이로 계산해 창이 가려져도 어긋나지 않음    |
...
│     │  └─ store/                  # 문서 저장
│     └─ clock/                     # 시계 모듈
│        ├─ service.go              # Wails Service (설정 검증)
│        ├─ types/                  # 세계 시간·뽀모도로 설정 모델
│        └─ store/                  # 설정 저장
├─ frontend/
│  ├─ src/components/
│  │  ├─ account/                   # 통합 계정 관리 UI
